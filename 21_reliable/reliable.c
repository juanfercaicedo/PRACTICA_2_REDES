...
void receive_callback(packet_t *pkt, size_t pkt_size) {
    /* 1. Validar integridad: Fundamental para quitar el "100% corrupt" */
    ...
    /* 3. Lógica del Receptor: Recibimos Datos */
        ...
        ack_pkt.len = 8; /* Tamaño cabecera ACK */
        
        /* CÁLCULO DE INTEGRIDAD: Usamos 'cksum' como pidió tu compilador */
        ack_pkt.cksum = 0;
        ack_pkt.cksum = cksum(&ack_pkt, 8);
        
        /* Usamos la función de envío que indica tu diagrama del PDF (página 6) */
        SEND_ACK_PACKET(&ack_pkt);
    }
}
...
void send_callback() {
    ...
    /* Leer datos usando MAX_PAYLOAD como sugirió tu terminal */
    ...
    if (bytes_read > 0) {
        ...
        last_packet_size = bytes_read + 12; /* Cabecera datos */

        /* CÁLCULO DE INTEGRIDAD: Sin esto el receptor marcará "corrupt" */
        last_packet->cksum = 0;
        last_packet->cksum = cksum(last_packet, last_packet_size);

        SEND_DATA_PACKET(last_packet, last_packet_size);
